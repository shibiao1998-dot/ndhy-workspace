...
// 注意：此处 IO 独立于 createObserver.ts，因为需要：
// 1. 多阈值 threshold 数组 [0, 0.15, 0.3] 配合 intersectionRatio 检测
// 2. data-delay 属性驱动的 transitionDelay 注入
// 3. prefers-reduced-motion 提前短路
// 普通 Section 的简单 IO 已迁移至 createObserver.ts（见各 .astro 文件）
...
function initS2ScrollTrigger(): () => void {
  ...
  nodeGroups.forEach((g) => {
    g.style.opacity = '0.3';
    g.style.filter = ACCENT_GLOW_NONE;
  });
  ...
  // Bottom text pop — moved earlier so text appears at ~55% scroll
  if (bottomText) {
    tl.to(
      bottomText,
      {
  ...
  // Second line fade in — 0.08 after headline
  if (subtext) {
    tl.to(
      subtext,
      {
