...
> 不理解就不设计，不设计就不开发。三个文档是项目的地基，地基不稳不动工。（2026-03-18 老板决策）

**完整流程（不可跳步、不可并行未完成前置步骤的后续步骤）**：

```
Gate 1: 需求挖掘 → 📄 需求文档 → [老板确认] ✅
Gate 2: 产品专家出产品方案 → 📄 产品文档 → [老板确认] ✅
Gate 3: 架构师出架构设计 → 📄 架构文档 → [老板确认] ✅
开发: 按架构拆模块 → 逐模块开发 → 逐模块验收 → 全部完成后整体验收
```
...
**模块化串行开发（宪法级）**
- 按架构文档拆分模块，每个模块独立、边界清晰
- **一次只开发一个模块**，不允许并行多模块
- 每个模块开发完成后**单独验收**（功能 + 代码 + 测试）
- **验收通过后才能开发下一个模块**
- 全部模块完成后进行**整体验收**（集成测试 + 端到端验证）

**设计与后端并行模式（Gate 3 通过后）**
Gate 3 通过后、前端开发前，可启动两条并行 Track：
- **Track A（设计线）**：A1 体验设计专家出交互文档 → A2 UI设计专家出视觉原型（A1→A2 串行）
- **Track B（后端线）**：M1 数据管道开发（与设计工作零依赖）
- 两条 Track 汇合后才进入 M2（前端项目骨架）
- **原则**：设计和后端可并行，但 UX→UI 必须串行（UI 的输入来自 UX）；前端模块（M2-M8）仍然严格串行
...
- ❌ 模块验收未通过就开始下一个模块
- ❌ 跳过整体验收直接交付
- ❌ 没有 UX 交互文档和 UI 原型就开始前端开发

工作流概览：`需求进入 → Gate 1（需求文档 → 老板确认）→ Gate 2（产品文档 → 老板确认）→ Gate 3（架构文档 → 老板确认）→ 模块化串行开发（逐模块开发+验收）→ 整体验收 → 交付`
